...
<7> The `@Detail` annotation can be on a field or a no-arg method. Multiple non-null details are concatenated with a space.
...
2. The exception message would regularly make for a very good default `detail`. But it might contain sensitive data that might get exposed when an existing system adds Problem Details. Should we still use them and offer an option to disable it when migrating an existing code base? They already often get exposed in the stack traces generated by the application containers.
3. Do we need an annotation parameter `@Logging(withStackTrace = ON)` or `OFF` or default `AUTO`.
4. How do we distinguish custom from standard exceptions, so the default status is 400 vs. 500? Almost all custom exceptions will be in the 4xx range and 400 is a very good default. OTOH standard exceptions like NPE are a problem in the service and should be mapped to a 500. Current approach: packages that start with `java.` or `javax.` or `jakarta.` are 'standard'; everything else is 'custom'.
5. How do we handle problems that origin from a remote call? Should we wrap them in a 502 Bad Gateway?
6. It's possible to send problem details to the server, e.g. for asynchronous responses. Is this out of scope or should we mention it? The code on the client as well as on the server would have to look quite different.
7. Should we remove the `get` (and lowercase the first letter) from the name of an `@Extension` method?
